...
alpha=transpose(alpha(2,1:np));

load('theta_exp_1k_60s_p1_use');% Theta
%theta_degree=data_theta(:,3);
%theta=theta_degree*pi/180;

t=transpose(theta(1,1:np));

theta=transpose(theta(3,1:np));
...
% split data
ratio=0.6; % 50 % for training, 50 % for cross validation (AIC calculation)
ntr=round(ratio*nt);
nts=nt-ntr;
