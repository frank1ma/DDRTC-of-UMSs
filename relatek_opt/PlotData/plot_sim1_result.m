...
%
load('data/sim1_result.mat')
load('data/sim1_combval')
load('data/sim1_radval')
load('data/sim1_ptgval')
...
xlabel('time$(s)$')
set(f1Ax1,'YLim',[-0.5 1]);      % Y-axis
set(f1Ax1,'XLim',[0 20]);      % X-axis
if (TitleFlag)                   % title
...
xlabel('time$(s)$')
set(f1Ax2,'YLim',[-1.5 1]);    % Y-axis
set(f1Ax2,'XLim',[0 20]);      % X-axis
if (TitleFlag)
...
f3ax.YAxis(2).Limits=[0 0.7];
f3ax.XAxis.Limits=[-0.8981 -0.8941];
%legend
legend('Relative Degree','Data Usage','Orientation','vertical')
% arrow position
...
annotation('arrow',[ start_point_x,end_point_x],[start_point_y,end_point_y]);           
% add text
annotation('textbox',[ start_point_x-0.12,start_point_y+0.024,0.1,0.1],'String',f2text,'LineStyle','none','Interpreter','latex','FitBoxToText','on'); 