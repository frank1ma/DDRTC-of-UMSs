function [rdval,combval,ptgval] = RelaTek(dim,t,raw_output,raw_input,srch_opts,crit_opts,win_opts,filt_opts)
...
%  hold on
% plot(log10(f),log10(abs((estf_data(:,1)))));
% grid on
    ...
    case 'manul'
        ...
        switch ans_select
            ...
            case 'Go select'               % select intervals
                figure;
                plot(log10(f),log10(estf_data(:,1)));
                grid on;
...
%crit_opts{2} =[interval_start,interval_end];  % final interval

faxis = abs(log10(f(interval_start:interval_end,1)));
%psd_data = psd_data(interval_start:interval_end,:);
%psd_u =  psd_u(interval_start:interval_end);

% construct matrix for data  num_of_numbers by 2  
...
parfor ii = 1:srch_len   
   ...
   [rd,ptg] = getrd_mex(dim,estf_feed_data,faxis,min_num_seg);
   rdarr(ii,:) =[rd ptg];   
end
